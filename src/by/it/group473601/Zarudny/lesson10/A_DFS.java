package by.it.group473601.Zarudny.lesson10;
...
public class A_DFS {
	...
	private static void DFS(int node, int graph[][], int n, int preVisit[], int postVisit[], int color[],
							int[][] edgeType) {
	...
	public static void print(int[] preVisit, int[] postVisit, int[][] edgeType) {
		System.out.println();
		for (int aPreVisit : preVisit) {
			System.out.print(aPreVisit + ", ");
		}
		System.out.println();
		for (int aPostVisit : postVisit) {
			System.out.print(aPostVisit + ", ");
		}
		System.out.println();
	...
	InputStream stream = new FileInputStream("C:\\Users\\Колобок\\IdeaProjects\\PISL2017-01-26\\src\\by\\it\\group473601\\vabishchevich\\lesson10\\dataA.txt");
