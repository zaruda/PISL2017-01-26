...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for(int i=0; i<s.length(); i++){
            elem=s.charAt(i);
            Integer value=count.get(elem);
            count.put(elem, value == null? 1: ++value);
        }

        for (Map.Entry entry : count.entrySet()) {
            priorityQueue.add(new LeafNode((int)entry.getValue(), (char)entry.getKey()));
        }

        while (priorityQueue.size()!=1){
            Node firstChild = priorityQueue.poll();
            Node secondChild = priorityQueue.poll();
            InternalNode parent = new InternalNode(firstChild, secondChild);
